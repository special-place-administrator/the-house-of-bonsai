...
extern void quantize_row_rq3_0_ref(const float * x, void * y, long long k);
extern void dequantize_row_rq3_0(const void * x, float * y, long long k);
extern void quantize_row_rq4_0_ref(const float * x, void * y, long long k);
extern void dequantize_row_rq4_0(const void * x, float * y, long long k);
...
int main(void) {
    ...
    printf("=== RotorQuant C Round-Trip Test ===\n\n");

    /* Test 1: basis vector */
    memset(input, 0, sizeof(input));
    input[0] = 1.0f;
    quantize_row_rq3_0_ref(input, buf, d);
    dequantize_row_rq3_0(buf, output, d);
    printf("Test 1 (rq3): e0 = [1, 0, ...]\n");
    ...
    quantize_row_rq4_0_ref(input, buf, d);
    dequantize_row_rq4_0(buf, output, d);
    printf("Test 3 (rq4): cos*5\n");
    printf("  In:  [%.4f, %.4f, %.4f, %.4f]\n", input[0], input[1], input[2], input[3]);
    printf("  Out: [%.4f, %.4f, %.4f, %.4f]\n", output[0], output[1], output[2], output[3]);
    mse = cosv = ni = no = 0;
    for (int i = 0; i < d; i++) { mse += (input[i]-output[i])*(input[i]-output[i]); cosv += input[i]*output[i]; ni += input[i]*input[i]; no += output[i]*output[i]; }
