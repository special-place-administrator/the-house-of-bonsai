...
static __global__ void flash_attn_ext_vec(
    ...
    constexpr bool is_rq_K = type_K == GGML_TYPE_RQ2_0 || type_K == GGML_TYPE_RQ3_0 || type_K == GGML_TYPE_RQ4_0 || type_K == GGML_TYPE_RQ3_ISO || type_K == GGML_TYPE_RQ4_ISO;
    constexpr bool is_rq_V = type_V == GGML_TYPE_RQ2_0 || type_V == GGML_TYPE_RQ3_0 || type_V == GGML_TYPE_RQ4_0 || type_V == GGML_TYPE_RQ3_ISO || type_V == GGML_TYPE_RQ4_ISO;
    constexpr int nthreads_KQ = (type_K == GGML_TYPE_F16 || type_K == GGML_TYPE_BF16 || is_rq_K) ? 128 / cpy_nb : nthreads_KQ_q;
    constexpr int nthreads_V  = (type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_BF16 || is_rq_V) ? 128 / cpy_nb : nthreads_V_q;

    static_assert(WARP_SIZE % nthreads_KQ == 0, "bad nthreads_K");
    static_assert(WARP_SIZE % nthreads_V  == 0, "bad nthreads_V");

    constexpr int V_rows_per_thread = (type_V == GGML_TYPE_F16 || type_V == GGML_TYPE_BF16 || is_rq_V) ? 2*cpy_ne : 4;
    constexpr int V_cols_per_iter   = WARP_SIZE / nthreads_V;

    constexpr vec_dot_KQ_t vec_dot_KQ = get_vec_dot_KQ<type_K, D, nthreads_KQ>();
    constexpr bool Q_q8_1 = type_K != GGML_TYPE_F16 && type_K != GGML_TYPE_BF16 && !is_rq_K;
    ...
    constexpr bool is_tcq3 = type_K == GGML_TYPE_RQ3_ISO || type_V == GGML_TYPE_RQ3_ISO;
    constexpr bool is_tcq2 = type_K == GGML_TYPE_RQ4_ISO || type_V == GGML_TYPE_RQ4_ISO;
    ...
    if constexpr (smem_cb_size > 0) {
        const float * cb_src = is_tcq3 ? d_rq3_iso_codebook_fattn : d_rq4_iso_codebook_fattn;
...
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
                float sum;
                if constexpr (type_K == GGML_TYPE_RQ3_ISO) {
                    sum = vec_dot_fattn_vec_KQ_rq3_iso_cb<D, nthreads_KQ>(
                        K + i_KQ*nb11, Q_reg[j], Q_i32[j], Q_ds[j], smem_codebook);
                } else if constexpr (type_K == GGML_TYPE_RQ4_ISO) {
                    sum = vec_dot_fattn_vec_KQ_rq4_iso_cb<D, nthreads_KQ>(
                        K + i_KQ*nb11, Q_reg[j], Q_i32[j], Q_ds[j], smem_codebook);
                } else {
...
#pragma unroll
                    ...
                    dequantize_V_rq3_iso_cb<half, V_rows_per_thread>(V + k*nb21, tmp,
                    ...
                    dequantize_V_rq4_iso_cb<half, V_rows_per_thread>(V + k*nb21, tmp,
                        2*i_VKQ_0 + (nthreads_V == WARP_SIZE ? threadIdx.x : threadIdx.x % nthreads_V)*V_rows_per_thread, smem_codebook);
                } else {
...
#pragma unroll
            for (int i_VKQ_0 = 0; i_VKQ_0 < D/2; i_VKQ_0 += nthreads_V*V_rows_per_thread/2) {
                ...
                if constexpr (type_V == GGML_TYPE_RQ3_ISO) {
                    dequantize_V_rq3_iso_cb<float, V_rows_per_thread>(V + k*nb21, tmp,
                        2*i_VKQ_0 + (nthreads_V == WARP_SIZE ? threadIdx.x : threadIdx.x % nthreads_V)*V_rows_per_thread, smem_codebook);
                } else if constexpr (type_V == GGML_TYPE_RQ4_ISO) {
                    dequantize_V_rq4_iso_cb<float, V_rows_per_thread>(V + k*nb21, tmp,
                        2*i_VKQ_0 + (nthreads_V == WARP_SIZE ? threadIdx.x : threadIdx.x % nthreads_V)*V_rows_per_thread, smem_codebook);
                } else {
                    dequantize_V(V + k*nb21, tmp,
                        2*i_VKQ_0 + (nthreads_V == WARP_SIZE ? threadIdx.x : threadIdx.x % nthreads_V)*V_rows_per_thread);
...
#define EXTERN_DECL_FATTN_VEC_CASES(D, type_K)             \
    ...
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_RQ2_0); \
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_RQ3_0); \
    extern DECL_FATTN_VEC_CASE(D, type_K, GGML_TYPE_RQ4_0); \
...
EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_RQ2_0)
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_RQ2_0)
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_RQ2_0)
EXTERN_DECL_FATTN_VEC_CASES(512, GGML_TYPE_RQ2_0)

EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_RQ3_0)
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_RQ3_0)
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_RQ3_0)
EXTERN_DECL_FATTN_VEC_CASES(512, GGML_TYPE_RQ3_0)

EXTERN_DECL_FATTN_VEC_CASES( 64, GGML_TYPE_RQ4_0)
EXTERN_DECL_FATTN_VEC_CASES(128, GGML_TYPE_RQ4_0)
EXTERN_DECL_FATTN_VEC_CASES(256, GGML_TYPE_RQ4_0)
EXTERN_DECL_FATTN_VEC_CASES(512, GGML_TYPE_RQ4_0)
