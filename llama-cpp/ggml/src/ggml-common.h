...
// RotorQuant 3-bit MSE-only: 3-bit PolarQuant indices (no QJL)
...
#define QK_RQ3 32   // Block size 32: matches q4_0 parallelism, graph handles WHT rotation
#define QK_RQ3_GROUP 128  // rotation group size = head_dim
typedef struct {
    ggml_half  norm;                    //  2 bytes: vector L2 norm (for rescaling)
    uint8_t    qs[QK_RQ3 / 4];      //  8 bytes: lower 2-bit indices (4 per byte)
    uint8_t    signs[QK_RQ3 / 8];   //  4 bytes: upper 1-bit of 3-bit index (8 per byte)
} block_rq3_0;                       // 14 bytes total
static_assert(sizeof(block_rq3_0) == sizeof(ggml_half) + QK_RQ3/4 + QK_RQ3/8, "wrong rq3_0 block size/padding");

// RotorQuant 2-bit: 2-bit PolarQuant indices, no QJL
// Per block: norm(fp16) + 2-bit indices (8 bytes) = 10 bytes per 32 values
// = 2.5 bits/value → 6.4× compression vs fp16
#define QK_RQ2 32
#define QK_RQ2_GROUP 128
typedef struct {
    ggml_half  norm;                    //  2 bytes: corrected vector L2 norm
    uint8_t    qs[QK_RQ2 / 4];      //  8 bytes: 2-bit indices (4 per byte)
} block_rq2_0;                       // 10 bytes total
static_assert(sizeof(block_rq2_0) == sizeof(ggml_half) + QK_RQ2/4, "wrong rq2_0 block size/padding");

// RotorQuant 3-bit TCQ: Trellis-Coded Quantization (right-shift bitshift trellis, k=3, L=9)
...
} block_rq3_iso;                     // 52 bytes total for 128 values (3.25 bpv)
static_assert(sizeof(block_rq3_iso) == sizeof(ggml_half) + 50, "wrong rq3_iso block size/padding");

// RotorQuant 2-bit TCQ: Trellis-Coded Quantization (right-shift bitshift trellis, k=2, L=8)
...
#define QK_RQ4_ISO 128
typedef struct {
    ggml_half  norm;                    //  2 bytes: corrected group L2 norm
    uint8_t    qs[33];                  // 33 bytes: 262-bit trellis bitstream (2 padding bits)
    uint8_t    pad;                     //  1 byte:  alignment padding
} block_rq4_iso;                     // 36 bytes total for 128 values (2.25 bpv)
static_assert(sizeof(block_rq4_iso) == sizeof(ggml_half) + 34, "wrong rq4_iso block size/padding");

// RotorQuant 4-bit: 16-level PolarQuant (Lloyd-Max optimal for post-WHT Gaussian)
...
typedef struct {
    ...
    uint8_t    qs[QK_RQ4 / 2];      // 64 bytes: 4-bit indices (2 per byte, low nibble first)
} block_rq4_0;                       // 66 bytes total
static_assert(sizeof(block_rq4_0) == sizeof(ggml_half) + QK_RQ4/2, "wrong rq4_0 block size/padding");
